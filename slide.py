...
def ppt_to_images(pptFilePath):
    ppt = win32com.client.Dispatch("PowerPoint.Application")
    presentation = ppt.Presentations.Open(pptFilePath, WithWindow=False)
    images = []
...
imgList = ppt_to_images(pptFilePath)
...
while True:
    ...
    if hands and buttonPressed is False:  # If hand is detected
        ...
        if fingers == [0, 1, 1, 0, 0]:  # Gesture for drawing
        ...
        if fingers == [0, 1, 1, 1, 0]:  # Gesture for erasing
            if annotations:
                annotations.pop(-1)
                annotationNumber -= 1
                buttonPressed = True
